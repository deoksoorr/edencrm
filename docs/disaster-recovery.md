...
### 경고 2 — 2026-07-29 이전 백업은 그대로 복구되지 않는다
...
| 종류 | 경로 | 생성 도구 | 주기 |
|---|---|---|---|
| 파일 | `database/backups/ftp_<YYYYMMDD-HHMMSS>/` | `deploy/backup.sh` (lftp mirror) | **수동 — 배포 직전에만** |
| DB | `database/backups/proddb_<label>_<YYYYMMDD-HHMMSS>.sql` | `deploy/db_dump.php` | **수동 — 배포 직전에만** |

**짝 맞추기**: 파일 백업과 DB 백업은 각각 독립 실행되므로 시각이 다를 수 있다.
파일명 시각으로 가장 가까운 쌍을 고르되, 덤프 첫 줄의 주석에 적힌 ISO 시각을 신뢰한다.

```bash
head -1 database/backups/proddb_*.sql   # 실제 생성 시각(ISO)
ls -ld database/backups/ftp_*           # 파일 백업 시각
```
...
| # | 문제 | 위험 | 상태 |
|---|---|---|---|
| 1 | 백업이 맥북 1대에만 존재 (git 원격 0개, 오프사이트 사본 0, `.gitignore` 로 git 제외) | 디스크 고장·분실 시 **코드와 백업 동시 전손 → 복구 불가** | ❗ 미해결 |
| 2 | 백업이 100% 수동, 배포 직전에만 실행 (cron·LaunchAgent 0건). 실측 최대 간격 **94.9시간** | 배포 없는 기간의 데이터는 백업되지 않음 | ❗ 미해결 |
| 3 | 관리자 비밀번호 보관 체계 없음 (`ADMIN_CREDENTIALS.local.txt` 무효) | 복구해도 **로그인 불가** | ❗ 미해결 |
| 4 | `rollback.sql` 이 39/46 테이블만 커버 (7개 누락) | 롤백 후 잔여 테이블로 충돌 | ❗ 미해결 |
| 5 | 백업에 운영 DB 자격증명 평문 포함(mode 644) | 백업 사본 유출 = 자격증명 유출 | ⚠ 복구 시 §3-1 로 대응 |
| 6 | `deploy.sh` 가 원격 삭제 미동기화 | 운영에 죽은 파일 누적(현재 4개, 웹 접근은 차단됨) | ⚠ 낮음 |
| ~~7~~ | ~~생성 컬럼 INSERT 로 전 백업 복구 불가~~ | ~~치명~~ | ✅ 2026-07-29 수정 |
| ~~8~~ | ~~charset 선언 없음~~ | ~~한글 파손~~ | ✅ 2026-07-29 수정 |
| ~~9~~ | ~~비원자적 덤프~~ | ~~정합성 깨진 백업~~ | ✅ 2026-07-29 수정 |
| ~~10~~ | ~~타임존 불일치로 파일명 9시간 어긋남~~ | ~~잘못된 백업쌍 선택~~ | ✅ 2026-07-29 수정 |
