                ...
                <td class="nowrap">
                  <?php if (!$isCancelled): ?>
                    ...
                    <button type="button" class="btn btn-sm btn-ghost btn-del-ppm" data-id="<?= (int) $pm['id'] ?>"
                      title="물리 삭제 대신 취소 상태로 전환 — 내역·감사 추적 보존">입금 무효</button>
...
(function () {
  ...
  document.querySelectorAll('.btn-del-ppm').forEach(function (btn) {
    btn.addEventListener('click', async function () {
      var ok = await EDEN.confirm('이 입금 내역을 무효 처리하시겠습니까? 내역은 삭제되지 않고 \'취소\' 상태로 보존되며, 누적 입금·확정 매출 집계에서 제외됩니다.', { danger: true, okLabel: '무효 처리' });
      if (!ok) return;
      try {
