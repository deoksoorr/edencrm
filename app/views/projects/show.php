...
<div class="page">
  ...
  <div class="card pad">
    ...
    <div class="kv-row mb-14">
      <div class="kv"><div class="kv-label">진행률</div><div class="kv-value"><?= $progress ?>%</div>
        <div class="progress mt-8" style="width:140px"><div class="progress-bar <?= $progress >= 100 ? 'ok' : ($isDelayed ? 'danger' : '') ?>" style="width:<?= $progress ?>%"></div></div>
      </div>
      <?php if ($canFinance): ?>
      <div class="kv"><div class="kv-label">계약금액</div><div class="kv-value"><?= moneyCell($calc['contract_amount']) ?></div></div>
      <div class="kv"><div class="kv-label">예상원가</div><div class="kv-value"><?= moneyCell($calc['estimated_cost']) ?></div></div>
      <div class="kv"><div class="kv-label">실제원가</div><div class="kv-value"><?= moneyCell($calc['actual_cost']) ?></div></div>
      <div class="kv"><div class="kv-label">예상순이익</div><div class="kv-value <?= $calc['estimated_profit'] < 0 ? 'text-danger' : '' ?>"><?= moneyCell($calc['estimated_profit']) ?></div></div>
      <div class="kv"><div class="kv-label">예상순이익률</div><div class="kv-value"><?= pct($calc['estimated_profit_rate']) ?></div></div>
      <div class="kv"><div class="kv-label">실제순이익</div><div class="kv-value <?= $calc['actual_profit'] < 0 ? 'text-danger' : '' ?>"><?= moneyCell($calc['actual_profit']) ?></div></div>
  ...
  <div class="card pad">
    <div class="section-head"><div class="st"><h2>배정 직원</h2></div></div>
    <?php if (!$assignments): ?>
          ...
          <tbody>
            <?php foreach ($assignments as $a): ?>
              <tr>
                <td><?= e($a['user_name']) ?></td>
                <td><?= e($a['role'] ?: '-') ?></td>
                <td class="num mono"><?= pct((float) $a['contribution_pct']) ?></td>
    ...
    <?php if ($canAssign): ?>
      <form method="post" action="<?= e(url('assignments.save')) ?>" class="form-grid mt-14">
        <?= csrf_field() ?>
  ...
  <div class="card pad">
    <div class="section-head"><div class="st"><h2>공정 이력</h2></div></div>
    <?php if (!$history): ?>
  ...
  <?php if ($canFinance): ?>
  <div class="card pad">
    <div class="section-head"><div class="st"><h2>비용</h2></div></div>
    <?php if (!$costs): ?>
          ...
          <tbody>
            <?php foreach ($costs as $c): ?>
              <tr>
                ...
                <td><?= e($c['category']) ?></td>
                <td class="num mono"><?= money((float) $c['amount']) ?></td>
    ...
    <?php if ($canCost): ?>
      <form method="post" action="<?= e(url('costs.save')) ?>" class="form-grid-3 mt-14">
        <?= csrf_field() ?>
        <input type="hidden" name="project_id" value="<?= (int) $p['id'] ?>">
  ...
  <div class="card pad">
    <div class="section-head"><div class="st"><h2>첨부 문서</h2></div></div>
    <?php if (!$docs): ?>
