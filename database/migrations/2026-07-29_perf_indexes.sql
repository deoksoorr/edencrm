...
-- 로컬(MySQL 9.6) 용 — 테이블명 prefix 없음. 운영용은 database/cafe24/014_perf_indexes.sql.
-- 적용: php scripts/apply_local_migration.php database/migrations/2026-07-29_perf_indexes.sql
--
...
SET @sql = IF(
  ...
  'DO 0',
  'ALTER TABLE `payments` ADD KEY `idx_payments_status_paid_date` (`status`, `paid_date`)');
...
SET @sql = IF(
  ...
  'DO 0',
  'ALTER TABLE `notifications` ADD KEY `idx_notifications_user_read` (`user_id`, `is_read`)');
...
SET @sql = IF(
  ...
  AND (SELECT COUNT(*) FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'notifications'
      AND INDEX_NAME = 'idx_notifications_user_read') > 0,
  'ALTER TABLE `notifications` DROP KEY `idx_notifications_is_read`',
  'DO 0');
...
SET @sql = IF(
  ...
  AND (SELECT COUNT(*) FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'payments'
      AND INDEX_NAME = 'idx_payments_status_paid_date') > 0,
  'ALTER TABLE `payments` DROP KEY `idx_payments_status`',
  'DO 0');
...
SET @sql = IF(
  ...
  AND (SELECT COUNT(*) FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'notifications'
      AND INDEX_NAME = 'idx_notifications_user_read') > 0,
  'ALTER TABLE `notifications` DROP KEY `idx_notifications_user`',
  'DO 0');
...
--   ANALYZE TABLE `payments`; ANALYZE TABLE `notifications`;
